...
Peep-MediaServer can be configured to ask clients for credentials, either in the form of username/password or a string-based token. These credentials are then validated through a chosen method.
...
12. In Peep-MediaServer, use the following JWKS URL:
...
If the `Authorization: Bearer` token cannot be directly provided (for instance, with web browsers that directly access Peep-MediaServer and show a credential dialog), you can pass the token as password, using an arbitrary user.
