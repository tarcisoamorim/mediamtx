...
Edit peep-mediaserver.yml and set the `rtmpEncryption`, `rtmpServerKey` and `rtmpServerCert` parameters:
...
Be aware that RTMPS is currently unsupported by all major players. However, you can use a local _Peep-MediaServer_ instance to decrypt streams before reading them, or alternatively a proxy like [stunnel](https://www.stunnel.org) or [nginx](https://nginx.org/). For instance, you can launch a local Peep-MediaServer instance with this configuration:
